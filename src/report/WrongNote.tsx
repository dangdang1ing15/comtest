...
function StudyText({ text, className = 'text-lg leading-loose text-slate-700' }: { text: string; className?: string }) {
            ...
            if (row.feedback)
              return (
                <div className="mt-4 rounded-2xl bg-indigo-50 p-5">
                  ...
                  <p className="mt-2 text-lg leading-loose text-indigo-900">{row.feedback.comment}</p>
                  <p className="mt-2 text-lg font-semibold text-indigo-700">👉 {row.feedback.actionPlan}</p>
                </div>
              );
            return null;
          })()}
...
export default function WrongNote({
  ...
  onReviewedChange: (level: Level, next: Set<number>) => void;
